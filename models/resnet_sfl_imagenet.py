...
class Local(nn.Module):
    def __init__(self, block, arch_cfg, groups, channels_per_group, norm, num_classes):
        super().__init__()
        self.in_channels = 64
        self.dilation = 1
        self.groups = groups
        ...
        if norm=='batch_norm':
            ...
            self.conv1 = nn.Conv2d(3, self.in_channels, (7, 7), (2, 2), (3, 3), bias=False)
            self.bn1 = nn.GroupNorm(1,self.in_channels)
            self.relu = nn.ReLU(True)
            self.maxpool = nn.MaxPool2d((3, 3), (2, 2), (1, 1))

        #we use a different inputsize than the original paper
        #so conv2_x's stride is 1
        self.layer1 = self._make_layer(arch_cfg[0], block, 64, norm, 1)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.in_channels != channels * block.expansion:
            if norm=='batch_norm':
                ...
                downsample = nn.Sequential(
                    nn.Conv2d(self.in_channels, channels * block.expansion, (1, 1), (stride, stride), (0, 0), bias=False),
                    nn.GroupNorm(1,channels * block.expansion),
                )
...
class Cloud(nn.Module):
    def __init__(self, block, arch_cfg, groups, channels_per_group, num_classes):
        super().__init__()
        self.in_channels = 64
        self.dilation = 1
        self.groups = groups
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.local = Local(block, arch_cfg, groups, channels_per_group, norm, num_classes)
        self.cloud = Cloud(block, arch_cfg, groups, channels_per_group, num_classes)
...
def resnet18(norm,num_classes=1000) -> ResNet:
    model = ResNet([2, 2, 2, 2], _BasicBlock, num_classes=num_classes, norm=norm)
